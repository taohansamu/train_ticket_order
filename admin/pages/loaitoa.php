<?php include("../template/header.php"); ?>
  ...
  <div class="w3-col m9">
    ...
    <form action="loaitoa.php" method="POST" class="w3-form">
         <h2 class="w3-text-teal" > Thêm Loại Toa </h2>
    <div class="w3-group">  
    <input class="w3-input" type="text"style="width:95%" name="maloaitoa" >
    <label class="w3-label">Mã Loại Toa</label>
  </div>
  <div class="w3-group">  
    <input class="w3-input " type="text"style="width:95%" name="tenloaitoa">
    <label class="w3-label">Tên Loại Toa</label>
  </div>
  <div class="w3-group">  
    <input class="w3-input " type="number"style="width:95%" step=0.25 name="heso">
    <label class="w3-label">Hệ Số Nhân</label>
  </div>
     ...
     if(isset($_POST["btn_add"]))
      {
        ...
        $heso=$_POST["heso"];
        if($maloaitoa=="" or $tenloaitoa=="")
          echo "KHÔNG ĐƯỢC BỎ TRỐNG TRƯỜNG NÀO !";
        else
          {
            
              $sql="SELECT * FROM loaitoa where maloaitoa='".$maloaitoa."'";
            ...
            {
              $sql="INSERT INTO loaitoa values('".$maloaitoa."','".$tenloaitoa."',".$heso.")";
              $query=pg_query($conn,$sql);
              $sql="SELECT * FROM loaitoa where maloaitoa='".$maloaitoa."'";
              $query=pg_query($conn,$sql);
              $num_row=pg_num_rows($query);
    ...
    <div class="container">
      <div class="row">
        <table class="w3-table w3-striped">
          <h2 style="color:teal"  >Danh sách loại toa</h2>
          <thead>
            <tr>
              <th>STT</th>
              <th>Mã Loại Toa</th>
          ...
          <tbody>
           ...
           <?php
            $stt = 1 ;
            $sql = "SELECT * FROM loaitoa order by maloaitoa";
            if(isset($_POST["btn_search"]))
            {
              ...
              $string_tenloaitoa=($tenloaitoa=="")?"1=0":"tenloaitoa='".$tenloaitoa."'";
              $string_heso=($heso=="")?"1=0":"heso='".$heso."'";
              $sql="select * from loaitoa where ".$string_maloaitoa." or ".$string_tenloaitoa." or ".$string_heso." order by maloaitoa";
            }
          ...
          ?>
            <tr>
              ...
              <td><?php echo $data["tenloaitoa"]; ?></td>
              <td><?php echo $data["heso"]; ?></td>
              <td><a href="../php/chinh-sua-loaitoa.php?id=<?php echo $data["maloaitoa"]; ?>">Sửa</a> <a href="../php/xoa-loaitoa.php?id=<?php echo $data["maloaitoa"]; ?>">Xóa</a></td>
            </tr>
          <?php
            }
          ?>
